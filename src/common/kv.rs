...
pub struct KvStore {
    /// A mapping of keys to their corresponding command positions in the log file.
    index: HashMap<String, CommandPosition>,

    /// A mapping of reader IDs to buffered file readers.
    current_readers: HashMap<u64, BufReader<File>>,

    /// The buffered writer for the current log file.
    current_writer: BufWriterWithPosition<File>,

    /// The current file number for the log file.
    current_file_number: u64,

    /// The directory path where the log files are stored.
    dir_path: PathBuf,

    /// The size of useless data in the log file.
    useless_size: u64,
}
